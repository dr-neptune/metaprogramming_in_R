...
The bquote function allows you to create expressions where you only partially quote -- you can evaluate some values when you create the expression and leave the other parts for later. 
...
In many situations where you need to create a call object, using bquote can be much simpler than constructing the call object explicitly. 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
# not a big diff for small examples
bquote(.(2 + 2) + y)
call("+", 2+2, quote(y))
...
You can also work with expressions as strings and translate between strings and expressions using the functions parse and deparse. 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
# to string
deparse(quote(x + y))

# take string and return expression
parse(text = "x + y")
...
The substitute function replaces variables for values in an expression. Getting the expression used as a function argument, rather than the value of the expression, is a common use of substitute. Together with deparse, it is used to create labels for plots. 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
# substitute doesn't get modified in the global environment
x <- 2; y <- 3
substitute(x + y)
...
When substitute looks in an environment, it does not follow the parent pointer. If it doesn't find the variable to substitute in the exact environment in which it is called, it will not look further. 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
# we can provide environments to substitute for it to look in
e <- new.env(parent = emptyenv())
e$x <- 2; e$y <- 3

substitute(x + y, e)
substitute(x + y, list(x = 2, y = 3))
substitute(x + y, list(x = 2, y = e))

# substitute does not follow parent pointers
x <- 3; y <- 3
...
A common case when you manipulate expressions is that you have a reference to an expression -- for example, from a function argument -- and you want to modify it. 

In the global environment, you cannot directly do this with substitute. 
...
# usually you manipulate expressions in a function
f <- function() {
    expr <- quote(x + y)
    substitute(expr)
}
...
When if you wish to do something like replace y with 2 here? 
...
# this will work. The order needs to be switched up
eval(substitute(substitute(expr, list(y = 2)), list(expr = expr)))
#+END_SRC

*** Substituting Function Arguments 

Function arguments are passed as unevaluated promises, but the second you access them, they get evaluated. If you want to get a hold of the promises without evaluating them, you can use the substitute function. This gives you the argument as a quoted expression. 

This can be useful if you want to manipulate expressions or evaluate them in ways different from the norm, but you do throw away information about which context the expression was supposed to be evaluated in. 
...
Inside R there is no mechanism for getting the environment out of a promise, so you cannot write code that modifies input expressions and then evaluates them in the enclosing scope for default parameters and the calling scope for function arugments.

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
# be careful when using substitute in functions that are called with other functions
...
NSE refers to any evaluation that doesn't follow the rules for how you evaluate expressions in the local evaluation environment. Typical uses of NSE are evaluating expressions in the calling scope, and evaluating expressions in data frames. 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
eval(quote(x + y), list(x = 2, y = 3))

# since a df is a list of vectors of the same length, we can do the same in dfs
d <- data.frame(x = 1:2, y = 3:3)
eval(quote(x + y), d)
#+END_SRC

To do NSE, you have to explicitly substitute an argument, so you do not evaluate the argument-promise in the calling scope and then evaluate it in an alternative scope. 
...
Here, the expression x + y is not quoted in the function call, so normally you'd expect x + y to evaluated in the calling scope. Because you explicitly use substitute to swap in the argument with my_with, this doesn't happen -- instead you evaluate the expression in the context of the data frame. This is NSE. 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
# if you use functions in the global scope, there is not a difference
z <- 1
with(d, x + y + z)
my_with(d, x + y + z)

# if you use them inside functions, there is
...
In my_with, there is a 3rd arg for the enclosing environment; the default here is the enclosing scope where eval is called. This is the global environment. In this case, z <- 1
In with, the enclosing environment is the calling environment, so z <- 2

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
# change my_with to mimic with behavior 
my_with <- function(df, expr) {
...
Now we have both typical uses of NSE: evaluation in a data frame, and evaluation in the calling scope. 

*** Nonstandard Evaluation from Inside Functions 

Since substitutions tend to work verbatim on the function arguments that you give functions, once arguments get passed from one function to another, NSE gets tricky:
...
If you want functions that do NSE, you should write functions that work with expressions and do normal evaluation on those instead. You can make a version of my_with that expects the expression to be already quoted, which you can use in other functions
...
Since NSE allows you to evaluate expressions in the calling scope, you can use it to write macros -- functions that work as shortcuts for statements where they are called. These differ from functions, which cannot generally modify data outside of their own scope. 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
# extract names from a list of function parameters and make sure all params have a name 
...
make_macro <- function(..., body) {
    ...
    # construct macro
    f <- eval(substitute(function() eval(substitute(body), parent.frame())))
...
# when m is called, it will eval x + y in calling scope
x <- 2; y <- 4
m()

# macro to replace specific values in a column in a dataframe with NA 
set_NA_val <- make_macro(df, var, na_val,
                         body = df$var[df$var == na_val] <- NA)

(d <- data.frame(x = c(1, -9, 3, 4),
                 y = c(1, 2, -9, -9)))
...
The reason you can modify variables in macros is that you can modify environments. Actual values are immutable, but when you modify the data frame in the previous example, you are replacing the reference in the environment to the modified data. 

Even with macros, you cannot actually modify data, but you can modify environments, and because you can evaluate expressions in environments different from the current evaluating environment you can make it appear as if you are modifying data in the calling environment. 

You can see this by examining the evaluation environment explicitly when you evaluate an assignment. 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
# modify e by setting z
e <- list2env(list(x = 2, y = 3))
eval(quote(z <- x + y), e)
as.list(e)

# only envs you can modify in lists; when eval an assignment in a list, you cannot modify the actual list
l <- list(x = 2, y = 3)
eval(quote(z <- x + y), l)
l
#+END_SRC
...
There is no mechanism in pure R to get access to the internals of promises, so if you use substitute to translate a function arg into its corresponding expression, you lose information about which environment the expression should be evaluated in. 
...
# after: rm env and calc value
g(x + y)
#+END_SRC

You can use the promise info to modify an environment and still evaluate it in the right scope. You just need to get hold of the code

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
# two ways
eval(substitute(substitute(code, list(y = quote(2 * y))),
                list(code = pi$code)))

eval(substitute(substitute(expr, list(y = quote(2 * y)))))
#+END_SRC

where expr is the parameter that holds the promise, and pi is the result of calling promise_info(expr). 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
f <- function(x, y) function(expr = x + y) {
    pi <- promise_info(expr)
    expr <- eval(substitute(substitute(expr, list(y = quote(2 * y)))))
...
Suppose you wanted to substitute whatever y is in the input expression with the y you have in the enclosing scope: 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
f <- function(x, y) function(expr = x + y) {
    pi <- promise_info(expr)
    expr <- eval(substitute(substitute(expr, list(y = 2 * y))))
    value <- eval(expr, pi$env)
